...
module.exports = {
	...
	create: async (context) => {
		const createdIssue = await issueLogics.createNewIssue(context.request.body, context.session);
		respond.success(context, createdIssue);
	},

	update: async (context) => {
		const updatedIssue = await issueLogics.updateIssue(context.request.body, context.session);
