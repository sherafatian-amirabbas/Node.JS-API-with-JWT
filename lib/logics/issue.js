...
module.exports = class Issue{
    ...
    async getRevisionDifferences(id_revisionA, id_revisionB, isDescending = false){
        const revisions = await RevisionEntity.findAll({
            where: {
                issueId: this.id,
                id: {
                    [Op.gte]: id_revisionA,
        ...
        const isAsc = (isDescending == false || isDescending === 'false' || isDescending === 'False' || 
            isDescending === '0' || isDescending === 0) ? true : false;
        const changes = isAsc ? issueA.subtract(issueB): issueB.subtract(issueA);
        ...
        const result = {
            ...
            changes: changes,
            revisions: revisions.map(u => Revision.fromModel(u))
        };
